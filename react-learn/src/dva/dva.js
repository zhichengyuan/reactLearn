...
export default function (opts = {}) {
    ...
    function runSubScriptions(dispatch){
        for (const model of app._models) {
            const newDispatch = function(action) {
                dispatch(geNewAction(action,model));
            }
     ...
     * @param {*} model 
     */
    function geNewAction(action,model) {
    ...
    function getMiddlewares() {
        ...
        getMiddlewares.runSaga = function (store) {
            ...
            for (const model of app._models) {
                const put = function(action) {//改造put函数,让它与模型关联
                    
                    return sagaEffects.put(geNewAction(action,model));
                }
                if(model.effects) {
